...
namespace WKCA.Com.Utils
{
    public class OutOfProcComRegistrationService : BaseComRegistrationService
    {
        ...
        protected override void UpdateDefaultRegistration(Assembly asm, string hostProcessPath)
        {
            ...
            {
                throw new ApplicationException("Hosting process path is required for out of proc COM server registration");
            }
        ...
        {
            using (var keyCLSID = Registry.ClassesRoot.OpenSubKey(string.Format(@"CLSID\{0}", t.GUID.ToString("B")), true))
            {
